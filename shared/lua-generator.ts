...
function luaKey(val: string): string {
  return /^[A-Za-z_][A-Za-z0-9_]*$/.test(val) ? val : `[${str(val)}]`;
}

function generateActionLua(
  actionType: string,
  ...
  return `{ type = '${actionType}', params = { ${tableEntries.join(", ")} } }`;
}

function generateThemeLua(name: string, palette: ColorPalette): string {
  const ansiStr = palette.ansi.map(str).join(", ");
  const brightsStr = palette.brights.map(str).join(", ");

  const indexedEntries: string[] = [];
  for (const [idx, color] of Object.entries(palette.indexed)) {
    indexedEntries.push(`[${idx}] = ${str(color)}`);
  }
  const indexedStr =
    indexedEntries.length > 0
      ? `indexed = { ${indexedEntries.join(", ")} },`
      : "";

  return `  ['${name.replace(/'/g, "\\'")}'] = {
    foreground = ${str(palette.foreground)},
    background = ${str(palette.background)},
    cursor_bg = ${str(palette.cursor_bg)},
    cursor_fg = ${str(palette.cursor_fg)},
    cursor_border = ${str(palette.cursor_border)},
    selection_fg = ${str(palette.selection_fg)},
    selection_bg = ${str(palette.selection_bg)},
    scrollbar_thumb = ${str(palette.scrollbar_thumb)},
    split = ${str(palette.split)},
    compose_cursor = ${str(palette.compose_cursor)},
    ansi = { ${ansiStr} },
    brights = { ${brightsStr} },
    ${indexedStr}
  }`;
...
function generatePaneLua(pane: PaneConfig): string {
  return `{ command = ${str(pane.command)}, shellType = ${str(pane.shellType)}, customShell = ${str(pane.customShell)} }`;
}

function generateExtraTabLua(tab: ExtraTabConfig): string {
  return `      { command = ${str(tab.command)}, title = ${str(tab.title)}, shellType = ${str(tab.shellType)}, customShell = ${str(tab.customShell)} },`;
}

function generateKeybindingLua(kb: Keybinding): string {
  const actionLua = generateActionLua(kb.action.type, kb.action.params);
  const disableDefault = kb.disableDefault ? ", disableDefault = true" : "";
...
export function generateLuaText(config: AgenticConfig, metadata?: any): string {
  ...
  const keybindingsLines = config.keybindings
    .filter((kb) => kb.enabled)
    .map((kb) => generateKeybindingLua(kb));
  const keybindingsBlock =
    keybindingsLines.length > 0 ? keybindingsLines.join(",\n") : "";
...
${metadataBlock}
  ...
  startup = {
    ...
    commandDelayMs = ${config.startup.commandDelayMs},
    layout = {
      type = ${str(config.startup.layout.type)},
      leftCommand = ${generatePaneLua(config.startup.layout.leftCommand)},
      rightTopCommand = ${generatePaneLua(config.startup.layout.rightTopCommand)},
      leftBottomCommand = ${generatePaneLua(config.startup.layout.leftBottomCommand)},
      rightBottomCommand = ${generatePaneLua(config.startup.layout.rightBottomCommand)},
    },
    extraTabs = {
${extraTabsLines.join("\n")}
    },
